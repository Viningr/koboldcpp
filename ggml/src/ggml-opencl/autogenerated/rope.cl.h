...
R"(        if (i0 >= n_offs && i0 < n_offs + n_dims) {
)"
R"(            int iw = i0 - n_offs; // relative idx
)"
R"(            int ic = iw/2;
)"
R"(
)"
R"(            float theta = theta_base * pow(freq_base, inv_ndims*iw);
)"
R"(
)"
...
R"(            float2 cos_sin_theta = rope_yarn(theta/freq_factor, freq_scale, corr_dims, iw, ext_factor, attn_factor);
)"
R"(
)"
...
R"(            const float theta = theta_base * pow(freq_base, inv_ndims*iw);
)"
R"(
)"
...
R"(            global float * src      = (global float *)((global char *) src0 + i3*nb03 + i2*nb02 + i1*nb01 + (n_offs + ic)*nb00);
)"
R"(            global float * dst_data = (global float *)((global char *)  dst + i3*nb3  + i2*nb2  + i1*nb1  + (n_offs + ic)*nb0);
)"
R"(
)"
...
R"(            global half * src       = (global half *)((global char *) src0 + i3*nb03 + i2*nb02 + i1*nb01 + (n_offs + ic)*nb00);
)"
R"(            global half * dst_data  = (global half *)((global char *)  dst + i3*nb3  + i2*nb2  + i1*nb1  + (n_offs + ic)*nb0);
)"
R"(
)"
R"(            const float x0 = src[0];
...
R"(            const int sector = ic % sect_dims;
)"
