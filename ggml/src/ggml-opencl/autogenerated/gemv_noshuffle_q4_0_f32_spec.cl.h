...
R"(        // Guard the two output rows against the padded x-grid tail overrunning dst.
)"
R"(        // The current shape specializations are all ne01 % 128 == 0 (no padding), so
)"
R"(        // this is a no-op / byte-identical today; keep it in lockstep with the base kernel.
)"
R"(        if (gid * 2 + 0 < ne01) dst[gid * 2 + 0] = totalSum.s0;
)"
R"(        if (gid * 2 + 1 < ne01) dst[gid * 2 + 1] = totalSum.s1;
)"
R"(    }
)"
R"(
