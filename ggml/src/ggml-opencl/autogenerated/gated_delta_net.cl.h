...
R"(    // input state holds s0 only [S_v, S_v, H, n_seqs]: per-seq stride is H*D.
)"
R"(    const uint state_base = (seq_id * H_v + head_id) * state_size;
)"
...
R"(    // snapshot slot mapping: slot 0 = most recent state, slot s = s tokens back.
)"
R"(    // When n_tokens < K only slots 0..n_tokens-1 are written; older slots are caller-owned.
)"
...
R"(            const int target_slot = (int)n_tokens - 1 - (int)t;
)"
