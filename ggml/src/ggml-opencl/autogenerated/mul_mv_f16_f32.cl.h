...
R"(    // The vector path below casts the row pointers to half4/float4, which must be 8- and
)"
R"(    // 16-byte aligned. A row address is r0*nb01 + ..., and a permuted or strided src leaves
)"
R"(    // nb01/nb11 unconstrained -- an odd ne00, say, gives a row that is only 2-byte aligned.
)"
R"(    // Every src1 row this work-item walks is src1_base + r1*nb11, so require both.
)"
R"(    const ulong src1_base  = (ulong) (src1 + (i12)*nb12 + (i13)*nb13);
)"
R"(    const bool  row_aligned = (((ulong) x) & 7) == 0 && (src1_base & 15) == 0 && (nb11 & 15) == 0;
)"
R"(
)"
R"(    if (ne00 < 128 || !row_aligned) {
)"
