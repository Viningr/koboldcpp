...
R"(// LOAD_VEC_A is 8 because one q1_0 quant byte expands to 8 weights along K.
)"
...
R"(kernel void kernel_mul_mm_q1_0_f32_l4_lm(
)"
...
R"(                int ib  = idx / 16; // 16 quant bytes per q1_0 block
)"
R"(
)"
R"(                float d = (float)src0_d[ib];
)"
R"(                uint bits = src0_q[idx];
)"
R"(
)"
R"(                // use float to avoid unsigned underflow of (2*0 - 1).
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 0) * BM + loadc_a + l] = d * (2.0f*(float)((bits >> 0) & 1) - 1.0f);
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 1) * BM + loadc_a + l] = d * (2.0f*(float)((bits >> 1) & 1) - 1.0f);
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 2) * BM + loadc_a + l] = d * (2.0f*(float)((bits >> 2) & 1) - 1.0f);
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 3) * BM + loadc_a + l] = d * (2.0f*(float)((bits >> 3) & 1) - 1.0f);
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 4) * BM + loadc_a + l] = d * (2.0f*(float)((bits >> 4) & 1) - 1.0f);
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 5) * BM + loadc_a + l] = d * (2.0f*(float)((bits >> 5) & 1) - 1.0f);
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 6) * BM + loadc_a + l] = d * (2.0f*(float)((bits >> 6) & 1) - 1.0f);
)"
R"(                buf_a[(loadr_a * LOAD_VEC_A + 7) * BM + loadc_a + l] = d * (2.0f*(float)((bits >> 7) & 1) - 1.0f);
)"
R"(            } else {
)"
R"(                for (int b = 0; b < LOAD_VEC_A; ++b) {
)"
R"(                    buf_a[(loadr_a * LOAD_VEC_A + b) * BM + loadc_a + l] = 0.0f;
)"
R"(                }
)"
R"(            }
