...
R"(kernel void kernel_swiglu_clamp(
)"
R"(    global char * src0,
)"
R"(    ulong         offset0,
...
R"(        const float gate = min(src0_row[i0], limit);
)"
R"(        const float up = clamp(src1_row[i0], -limit, limit);
)"
R"(
)"
R"(        dst_row[i0] = gate / (1.0f + exp(-gate)) * up;
)"
R"(    }
)"
R"(}
...
R"(kernel void kernel_swiglu_clamp_f16(
)"
R"(    global char * src0,
)"
R"(    ulong         offset0,
...
R"(        const float gate = min((float) src0_row[i0], limit);
)"
R"(        const float up = clamp((float) src1_row[i0], -limit, limit);
)"
R"(
)"
R"(        dst_row[i0] = (half) (gate / (1.0f + exp(-gate)) * up);
)"
R"(    }
)"
R"(}
