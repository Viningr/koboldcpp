...
R"(    // Take the vector path only when the rows this work-item touches are actually aligned;
)"
R"(    // the scalar loop has no such requirement.
)"
R"(    const bool row_aligned = (((ulong) x) & 7) == 0 && (((ulong) y) & 15) == 0;
)"
R"(
)"
R"(    float sumf = 0;
