...
R"(// per byte, value 0..15), packed for the int8 dp4a. The -8 zero-point is applied
)"
R"(// in the epilogue via the activation sum term (cheaper than biasing every byte).
)"
...
R"(// One token's dp4a dot (8 uints = 32 K elems) + q4_0 scale/zero-point epilogue.
)"
R"(#define MOE_Q40_DP4A_T(t) do {                                       \
)"
...
R"(        acc[t] += d_val * ((float)sh_d[t] * (float)raw - 8.0f * (float)sh_s[t]); \
)"
R"(    } while (0)
)"
R"(
...
R"(kernel void kernel_gemm_moe_q4_0_q8_1_dp4a(
)"
R"(        __read_only  image1d_buffer_t src0_q,   // q4_0 weights (transposed, packed nibbles)
)"
R"(        __global     half *           src0_d,   // per-32-block scale
)"
...
R"(        // per-32-block scale for this WI's row
)"
R"(        const uint d_offset = row_idx + sub * ne01 + expert_id * num_blocks * ne01;
)"
...
R"(            sh_s[lid] = src1_sa[(col + lid) * num_blocks + sub];
)"
R"(        }
)"
R"(        barrier(CLK_LOCAL_MEM_FENCE);
...
R"(            for (int t = 0; t < TILESIZE_N; ++t) { MOE_Q40_DP4A_T(t); }
)"
R"(        } else {
)"
R"(            #pragma unroll 4
)"
R"(            for (int t = 0; t < n_real; ++t) { MOE_Q40_DP4A_T(t); }
)"
R"(        }
)"
R"(        barrier(CLK_LOCAL_MEM_FENCE);
