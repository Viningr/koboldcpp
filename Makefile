...
OPENCL_FLAGS = -DGGML_USE_OPENCL -DGGML_OPENCL_TARGET_VERSION=300 -DGGML_OPENCL_SOA_Q -DGGML_OPENCL_USE_ADRENO_KERNELS -DGGML_OPENCL_EMBED_KERNELS -Iggml/src/ggml-opencl/autogenerated
OPENCL_STUB ?= android-opencl-stub/libOpenCL.so
OPENCL_LIB ?= $(OPENCL_STUB)
HEXAGON_SDK_ROOT ?= /opt/hexagon/6.6.0.0
HEXAGON_FLAGS = -DGGML_USE_HEXAGON -Iggml/src/ggml-hexagon/htp -Iggml/src/ggml-hexagon/generated -I$(HEXAGON_SDK_ROOT)/incs -I$(HEXAGON_SDK_ROOT)/incs/stddef -I$(HEXAGON_SDK_ROOT)/utils/examples -I$(HEXAGON_SDK_ROOT)/ipc/fastrpc/rtld/ship/android_aarch64
HEXAGON_LIB = -llog -ldl
ifdef LLAMA_CUBLAS
...
ggml_v4_opencl.o: ggml/src/ggml.c ggml/include/ggml.h
	$(CC)  $(FASTCFLAGS) $(FULLCFLAGS) $(OPENCL_FLAGS) -c $< -o $@
ggml_v4_hexagon.o: ggml/src/ggml.c ggml/include/ggml.h
	$(CC)  $(FASTCFLAGS) $(FULLCFLAGS) $(HEXAGON_FLAGS) -c $< -o $@
...
ggml-backend-reg_opencl.o: ggml/src/ggml-backend-reg.cpp ggml/src/ggml-backend-impl.h ggml/include/ggml.h ggml/include/ggml-backend.h ggml/include/ggml-cpu.h ggml/include/ggml-opencl.h
	$(CXX)  $(CXXFLAGS) $(OPENCL_FLAGS) -c $< -o $@
ggml-backend-reg_hexagon.o: ggml/src/ggml-backend-reg.cpp ggml/src/ggml-backend-impl.h ggml/include/ggml.h ggml/include/ggml-backend.h ggml/include/ggml-cpu.h ggml/include/ggml-hexagon.h
	$(CXX)  $(CXXFLAGS) $(HEXAGON_FLAGS) -c $< -o $@
ggml-backend-reg_snapdragon.o: ggml/src/ggml-backend-reg.cpp ggml/src/ggml-backend-impl.h ggml/include/ggml.h ggml/include/ggml-backend.h ggml/include/ggml-cpu.h ggml/include/ggml-opencl.h ggml/include/ggml-hexagon.h
	$(CXX)  $(CXXFLAGS) $(OPENCL_FLAGS) $(HEXAGON_FLAGS) -c $< -o $@
...
# Snapdragon Adreno OpenCL backend (current upstream llama.cpp source, embedded kernels)
ggml-opencl.o: ggml/src/ggml-opencl/ggml-opencl.cpp ggml/include/ggml-opencl.h
	$(CXX) $(CXXFLAGS) $(OPENCL_FLAGS) -c $< -o $@
cl-program-cache.o: ggml/src/ggml-opencl/cl-program-cache.cpp ggml/src/ggml-opencl/cl-program-cache.h
	$(CXX) $(CXXFLAGS) $(OPENCL_FLAGS) -c $< -o $@

# Snapdragon Hexagon/HTP backend (current upstream llama.cpp source)
ggml-hexagon.o: ggml/src/ggml-hexagon/ggml-hexagon.cpp ggml/include/ggml-hexagon.h ggml/src/ggml-hexagon/generated/htp_iface.h
	$(CXX) $(CXXFLAGS) $(HEXAGON_FLAGS) -c $< -o $@
htp-drv.o: ggml/src/ggml-hexagon/htp-drv.cpp ggml/src/ggml-hexagon/htp-drv.h
	$(CXX) $(CXXFLAGS) $(HEXAGON_FLAGS) -c $< -o $@
htp_iface_stub.o: ggml/src/ggml-hexagon/generated/htp_iface_stub.c ggml/src/ggml-hexagon/generated/htp_iface.h
	$(CC) $(CFLAGS) $(HEXAGON_FLAGS) -c $< -o $@
...
kcpp_backend_opencl.o: kcpp_backend.cpp kcpp_backend.h
	$(CXX) $(CXXFLAGS) $(OPENCL_FLAGS) -c $< -o $@
kcpp_backend_snapdragon.o: kcpp_backend.cpp kcpp_backend.h
	$(CXX) $(CXXFLAGS) $(OPENCL_FLAGS) $(HEXAGON_FLAGS) -c $< -o $@

$(OPENCL_STUB): android-opencl-stub/opencl-stub.c
	$(CC) $(CFLAGS) -shared -fPIC -Wl,-soname,libOpenCL.so $< -o $@

clean:
	rm -vf *.o main ttsmain sdmain whispermain quantize_gguf quantize_gpt2 quantize_gptj quantize_neox quantize_mpt vulkan-shaders-gen vulkan-shaders-gen-noext gguf-split mtmd-cli mainvk mainopencl mainhexagon mainsnapdragon fitparams embedding embeddingvk qwen3tts qwen3ttssnapdragon rpcserver llamaserver llamaservervk rpcserver.exe llamaserver.exe llamaservervk.exe qwen3tts.exe embeddingvk.exe embedding.exe fitparams.exe mainvk.exe mtmd-cli.exe gguf-split.exe vulkan-shaders-gen.exe vulkan-shaders-gen-noext.exe main.exe ttsmain.exe sdmain.exe whispermain.exe quantize_gguf.exe quantize_gptj.exe quantize_gpt2.exe quantize_neox.exe quantize_mpt.exe koboldcpp_default.dll koboldcpp_failsafe.dll koboldcpp_noavx2.dll koboldcpp_vulkan_failsafe.dll koboldcpp_cublas.dll koboldcpp_hipblas.dll koboldcpp_vulkan.dll koboldcpp_vulkan_noavx2.dll koboldcpp_default.so koboldcpp_snapdragon.so koboldcpp_failsafe.so koboldcpp_macos_failsafe.so koboldcpp_noavx2.so koboldcpp_vulkan_failsafe.so koboldcpp_cublas.so koboldcpp_hipblas.so koboldcpp_vulkan.so koboldcpp_vulkan_noavx2.so ggml/src/ggml-vulkan-shaders.cpp ggml/src/ggml-vulkan-shaders.hpp ggml/src/ggml-vulkan-shaders-noext.cpp ggml/src/ggml-vulkan-shaders-noext.hpp $(OPENCL_STUB)
...
mainopencl: tools/completion/main.cpp tools/completion/completion.cpp common/arg.cpp common/preset.cpp $(COMMON_DOWNLOAD_SRCS) build-info.h ggml_v4_opencl.o ggml-cpu.o ggml-ops.o ggml-vec.o ggml-binops.o ggml-unops.o llama.o chat.o llama-model.o console.o clip_default.o mtmd.o mtmd-helper.o mtmd-helper-gen.o mtmd-image.o ggml-backend.o ggml-backend-meta.o ggml-backend-reg_opencl.o ggml-opencl.o cl-program-cache.o ggml-repack.o $(OBJS_FULL) $(OBJS) | $(OPENCL_STUB)
	$(CXX) $(CXXFLAGS) $(OPENCL_FLAGS) $(filter-out %.h,$^) $(OPENCL_LIB) -o $@ $(LDFLAGS)
mainhexagon: tools/completion/main.cpp tools/completion/completion.cpp common/arg.cpp common/preset.cpp $(COMMON_DOWNLOAD_SRCS) build-info.h ggml_v4_hexagon.o ggml-cpu.o ggml-ops.o ggml-vec.o ggml-binops.o ggml-unops.o llama.o chat.o llama-model.o console.o clip_default.o mtmd.o mtmd-helper.o mtmd-helper-gen.o mtmd-image.o ggml-backend.o ggml-backend-meta.o ggml-backend-reg_hexagon.o ggml-hexagon.o htp-drv.o htp_iface_stub.o ggml-repack.o $(OBJS_FULL) $(OBJS)
	$(CXX) $(CXXFLAGS) $(HEXAGON_FLAGS) $(filter-out %.h,$^) $(HEXAGON_LIB) -o $@ $(LDFLAGS)
mainsnapdragon: tools/completion/main.cpp tools/completion/completion.cpp common/arg.cpp common/preset.cpp $(COMMON_DOWNLOAD_SRCS) build-info.h ggml_v4_opencl.o ggml-cpu.o ggml-ops.o ggml-vec.o ggml-binops.o ggml-unops.o llama.o chat.o llama-model.o console.o clip_default.o mtmd.o mtmd-helper.o mtmd-helper-gen.o mtmd-image.o ggml-backend.o ggml-backend-meta.o ggml-backend-reg_snapdragon.o ggml-opencl.o cl-program-cache.o ggml-hexagon.o htp-drv.o htp_iface_stub.o ggml-repack.o $(OBJS_FULL) $(OBJS) | $(OPENCL_STUB)
	$(CXX) $(CXXFLAGS) $(OPENCL_FLAGS) $(HEXAGON_FLAGS) $(filter-out %.h,$^) $(OPENCL_LIB) $(HEXAGON_LIB) -o $@ $(LDFLAGS)
...
qwen3tts: otherarch/qwen3tts/q3ttsmain.cpp otherarch/qwen3tts/qwen3_tts.cpp otherarch/qwen3tts/text_tokenizer.cpp otherarch/qwen3tts/gguf_loader.cpp otherarch/qwen3tts/tts_transformer.cpp otherarch/qwen3tts/audio_tokenizer_decoder.cpp otherarch/qwen3tts/audio_tokenizer_encoder.cpp otherarch/qwen3tts/coreml_code_predictor_stub.cpp ggml.o ggml-cpu.o ggml-ops.o ggml-vec.o ggml-binops.o ggml-unops.o llama.o chat.o llama-model.o console.o clip_default.o mtmd.o mtmd-helper.o mtmd-helper-gen.o mtmd-image.o ggml-backend.o ggml-backend-meta.o ggml-backend-reg_default.o ggml-repack.o $(OBJS_FULL) $(OBJS)
	$(CXX) $(CXXFLAGS) $(filter-out %.h,$^) -o $@ $(LDFLAGS)
qwen3ttssnapdragon: otherarch/qwen3tts/q3ttsmain.cpp otherarch/qwen3tts/qwen3_tts.cpp otherarch/qwen3tts/text_tokenizer.cpp otherarch/qwen3tts/gguf_loader.cpp otherarch/qwen3tts/tts_transformer.cpp otherarch/qwen3tts/audio_tokenizer_decoder.cpp otherarch/qwen3tts/audio_tokenizer_encoder.cpp otherarch/qwen3tts/coreml_code_predictor_stub.cpp ggml_v4_opencl.o ggml-cpu.o ggml-ops.o ggml-vec.o ggml-binops.o ggml-unops.o llama.o chat.o llama-model.o console.o clip_default.o mtmd.o mtmd-helper.o mtmd-helper-gen.o mtmd-image.o ggml-backend.o ggml-backend-meta.o ggml-backend-reg_snapdragon.o ggml-opencl.o cl-program-cache.o ggml-hexagon.o htp-drv.o htp_iface_stub.o ggml-repack.o $(OBJS_FULL) $(OBJS) | $(OPENCL_STUB)
	$(CXX) $(CXXFLAGS) $(OPENCL_FLAGS) $(HEXAGON_FLAGS) $(filter-out %.h,$^) $(OPENCL_LIB) $(HEXAGON_LIB) -o $@ $(LDFLAGS)
...
koboldcpp_snapdragon: ggml_v4_opencl.o ggml-cpu.o ggml-ops.o ggml-vec.o ggml_v3.o ggml_v2.o ggml_v1.o kcpp_backend_snapdragon.o ggml-backend-reg_snapdragon.o ggml-opencl.o cl-program-cache.o ggml-hexagon.o htp-drv.o htp_iface_stub.o $(KOBOLDCPP_COMMON_OBJS) $(OBJS_FULL) | $(OPENCL_STUB)
	$(CXX) $(CXXFLAGS) $(OPENCL_FLAGS) $(HEXAGON_FLAGS) $^ $(OPENCL_LIB) $(HEXAGON_LIB) -shared -o $@.so $(LDFLAGS)
