...
R"(__kernel void kernel_gemv_moe_mxfp4_f32_ns_wimg(
)"
...
R"(        regQ.s0 = read_imageui(src0_q, (int)(block_offset)).x;
)"
R"(        regQ.s1 = read_imageui(src0_q, (int)(block_offset + ne01)).x;
)"
R"(        regQ.s2 = read_imageui(src0_q, (int)(block_offset + ne01 * 2)).x;
)"
R"(        regQ.s3 = read_imageui(src0_q, (int)(block_offset + ne01 * 3)).x;
)"
R"(
)"
R"(        uint offset = i11 * ne00 / 4 + ib00 * 8;
